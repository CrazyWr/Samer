...
-(BOOL)updateMusicPlayer:(Music *)musicItem
{
    NSString *updateSql = @"update t_music set like=? where ID=?";
    BOOL ret = [self.db executeUpdate:updateSql,musicItem.db_id,musicItem.db_id];
    if (ret ==YES) {
        NSLog(@"修改成功");
        return YES;
    }
    ...
    {
        NSLog(@"修改失败，具体原因：%@",self.db.lastErrorMessage);
        return NO;
    }
