...
#import "MusicBarMusicView.h"

@interface MusicBarViewController ()<AVPlayerManageDelegate,UIScrollViewDelegate>

@property (nonatomic,strong) MusicPlayerListManage *musicPLManage;//音乐播放列表管理
...
@property (nonatomic,strong) MusicBarMusicView *musicView;//当前歌曲视图

@property (nonatomic,strong) MusicBarMusicView *lastMusicView;//上一首歌曲视图

@property (nonatomic,strong) MusicBarMusicView *nextMusicView;//下一首歌曲视图
...
- (void)viewDidLoad {
    ...
    self.avPlayer = [AVPlayerManage shareAVPlayerManage];
...
- (void)loadMusicBarView{
    ...
    _musicView = [[MusicBarMusicView alloc]initWithFrame:CGRectMake(CGRectGetWidth(_scrollView.frame), 0, CGRectGetWidth(_scrollView.frame), CGRectGetHeight(_scrollView.frame))];
    ...
    _lastMusicView = [[MusicBarMusicView alloc]initWithFrame:CGRectMake(0 , 0, CGRectGetWidth(_scrollView.frame), CGRectGetHeight(_scrollView.frame))];
    
    [_scrollView addSubview:_lastMusicView];
    
    //初始化下一首歌曲视图
    
    _nextMusicView = [[MusicBarMusicView alloc]initWithFrame:CGRectMake(CGRectGetWidth(_scrollView.frame) * 2 , 0, CGRectGetWidth(_scrollView.frame), CGRectGetHeight(_scrollView.frame))];
